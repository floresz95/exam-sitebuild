...
const header = () => {
    return `
        <header id="header">
            <span id="logo">New York Times Best Sellers</span>
...
const bCard = (number, title, sub, text) => {
    return `
        <div class="bookCard">
            ...
            <button>
                ...
                <span class="material-symbols-sharp" id="arrowIcon">arrow_forward</span>
            </button>
        </div>
    `
}

const createBCard = (obj) => {
    return obj.cards.map((b,i) => bCard(i+1, b.title, b.sub, b.text)).join(" ")
}

const loadEvent = () => {
    const rootEl = document.getElementById("root")
    rootEl.insertAdjacentHTML("beforeend", header())
    rootEl.insertAdjacentHTML("beforeend", mainDiv)

    const bCardDivEl =document.getElementById("mainDiv")
    bCardDivEl.insertAdjacentHTML("beforeend", createBCard(books))
}
